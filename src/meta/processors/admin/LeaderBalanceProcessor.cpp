...
cpp2::ErrorCode LeaderBalanceProcessor::leaderBalance() {
...
bool LeaderBalanceProcessor::buildLeaderBalancePlan(HostLeaderMap* hostLeaderMap,
...
bool LeaderBalanceProcessor::getHostParts(bool dependentOnGroup,
                                          HostParts& hostParts,
                                          int32_t& totalParts) {
    folly::SharedMutex::ReadHolder rHolder(LockUtils::spaceLock());
    auto prefix = MetaServiceUtils::partPrefix(space_);
...
bool LeaderBalanceProcessor::assembleZoneParts(const std::string& groupName, HostParts& hostParts) {
