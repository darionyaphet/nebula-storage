...
DownloadJobExecutor::DownloadJobExecutor(JobID jobId,
    ...
    : MetaJobExecutor(jobId, kvstore, adminClient, paras) {
        LOG(INFO) << "Download Job Executor init";
        taskParas_.resize(3);
...
nebula::cpp2::ErrorCode DownloadJobExecutor::prepare() {
    ...
    auto& hdfsHost = paras_[3];
    auto& hdfsPort = paras_[2];
    auto& hdfsPath = paras_[1];
    LOG(INFO) << "HDFS Host: " << hdfsHost
              << " HDFS Port: " << hdfsPort
              << " HDFS Path: " << hdfsPath;

    for (size_t i = 0; i < hdfsPort.length(); i++) {
        if (std::isdigit(hdfsPort[i]) == false) {
            LOG(ERROR) << "Port should be an integer";
            return nebula::cpp2::ErrorCode::E_DATA_TYPE_MISMATCH;
        }
    }

    auto listResult = helper_->ls(hdfsHost, folly::to<int32_t>(hdfsPort), hdfsPath);
    if (!listResult.ok()) {
        LOG(ERROR) << "Dispatch SSTFile Failed";
        return nebula::cpp2::ErrorCode::E_INVALID_JOB;
    }

    taskParas_.emplace_back(hdfsHost);
    taskParas_.emplace_back(hdfsPort);
    taskParas_.emplace_back(hdfsPath);
