...
TEST_F(GetBalancePlanTest, BalancePlanJob) {
    ...
    ASSERT_EQ(result, nebula::kvstore::ResultCode::SUCCEEDED);

    auto job = JobDescription::loadJobDescription(balanceJob.id_, kv_.get());
    ASSERT_TRUE(job);
    ASSERT_EQ(balanceJob.id_, job.value().id_);
    ASSERT_EQ(cpp2::JobStatus::QUEUE, job.value().status_);

    ASSERT_TRUE(job->setStatus(cpp2::JobStatus::RUNNING));
    ASSERT_EQ(result, nebula::kvstore::ResultCode::SUCCEEDED);

    BalancePlan plan = BalancePlan(balanceJob.id_, spaceId, kv_.get(), nullptr);

    for (int32_t i = 0; i < 3; i++) {
        plan.addTask(BalanceTask(balanceJob.id_, spaceId, i,
    ...
    ASSERT_EQ(cpp2::ErrorCode::SUCCEEDED, plan.saveJobStatus());

    {
        ...
        auto planItem = resp.plan;
        ASSERT_NE(cpp2::JobStatus::FINISHED, planItem.status);
    }
...

    {
        ...
        auto planItem = resp.plan;
        ASSERT_EQ(cpp2::JobStatus::FINISHED, planItem.status);
        ASSERT_EQ(3, planItem.tasks.size());
    }
}
