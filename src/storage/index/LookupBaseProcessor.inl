...
StatusOr<StoragePlan<IndexID>> LookupBaseProcessor<REQ, RESP>::buildPlan() {
    ...
    for (const auto& ctx : contexts_) {
        ...
        for (const auto& col : index.value()->get_fields()) {
            indexCols.emplace_back(col.get_name(),
                                   IndexKeyUtils::toValueType(col.get_type().get_type()));
            if (IndexKeyUtils::toValueType(col.get_type().get_type()) == Value::Type::STRING) {
                vColNum++;
            }
